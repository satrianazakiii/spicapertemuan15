...
$f_supplier = isset($_GET['supplier']) ? mysqli_real_escape_string($koneksi, $_GET['supplier']) : '';

$where_parts = [];
if ($f_dari)     $where_parts[] = "p.tanggal_pembelian >= '$f_dari'";
if ($f_sampai)   $where_parts[] = "p.tanggal_pembelian <= '$f_sampai'";
if ($f_supplier) $where_parts[] = "p.id_supplier = '$f_supplier'";
...
$result = mysqli_query($koneksi,
    "SELECT p.*, s.nama_supplier
     ...
     $where
     ORDER BY p.tanggal_pembelian DESC, p.no_pembelian DESC");
$total_rows = mysqli_num_rows($result);

// Grand total
$grand = mysqli_fetch_assoc(mysqli_query($koneksi,
    "SELECT SUM(total_barangall) AS total_b, SUM(total_hargaall) AS total_h
     FROM tb_pembelian p $where"));

// Supplier list untuk dropdown filter
$supplier_list = mysqli_query($koneksi, "SELECT id_supplier, nama_supplier FROM tb_supplier ORDER BY nama_supplier");
...
<head>
  ...
  <style>
    ...
    .badge-info { background:#17a2b8;color:#fff;padding:3px 8px;border-radius:4px;font-size:.78rem; }
    /* ── Print ── */
    @media print {
      ...
      .container-scroller, .container-fluid { display: block !important; }
    ...
    <ul class="nav">
      ...
      <li class="nav-item">
        <a class="nav-link" data-bs-toggle="collapse" href="#ui-basic"
           aria-expanded="<?= $open_data ? 'true' : 'false' ?>" aria-controls="ui-basic">
          <i class="mdi mdi-view-headline menu-icon"></i>
          <span class="menu-title">Kelola Data</span>
          <i class="menu-arrow"></i>
        </a>
        <div class="collapse <?= $open_data ?>" id="ui-basic">
      ...
      <li class="nav-item">
        <a class="nav-link" data-bs-toggle="collapse" href="#auth"
           aria-expanded="<?= $open_transaksi ? 'true' : 'false' ?>" aria-controls="auth">
          <i class="mdi mdi-view-headline menu-icon"></i>
          <span class="menu-title">Kelola Transaksi</span>
          <i class="menu-arrow"></i>
        </a>
        <div class="collapse <?= $open_transaksi ?>" id="auth">
          <ul class="nav flex-column sub-menu">
      ...
      <li class="nav-item">
        <a class="nav-link" data-bs-toggle="collapse" href="#auth2"
           aria-expanded="true" aria-controls="auth2">
          <i class="mdi mdi-view-headline menu-icon"></i>
    ...
    <div class="main-panel">
      <div class="content-wrapper">
        ...
        <div class="page-header mb-4 no-print">
          <h3 class="page-title">Laporan Transaksi Pembelian</h3>
        ...
        <div class="card mb-4 no-print">
          <div class="card-body py-3">
            <form method="GET" class="row g-2 align-items-end">
              ...
              <div class="col-auto">
                <label class="form-label mb-1 small fw-semibold">Supplier</label>
                <select name="supplier" class="form-select form-select-sm" style="min-width:160px;">
                  <option value="">Semua Supplier</option>
                  <?php while ($s = mysqli_fetch_assoc($supplier_list)): ?>
                  <option value="<?= $s['id_supplier'] ?>"
                    <?= $f_supplier == $s['id_supplier'] ? 'selected' : '' ?>>
              ...
              <div class="col-auto d-flex gap-2">
                ...
                <a href="laporan_pembelian.php" class="btn btn-outline-secondary btn-sm">Reset</a>
              </div>
        ...
        <div class="card">
          <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-3 no-print">
              <h4 class="card-title mb-0">Data Transaksi Pembelian</h4>
              ...
              <table class="table table-bordered table-hover align-middle">
                ...
                <tbody>
                  ...
                  <tr>
                    ...
                    <td><span class="badge badge-info"><?= htmlspecialchars($row['no_pembelian']) ?></span></td>
                    <td><?= htmlspecialchars($row['tanggal_pembelian']) ?></td>
                    <td><?= htmlspecialchars($row['nama_supplier'] ?? $row['id_supplier']) ?></td>
